...
@implementation SentryLogWindowC
- init
{
    if (self = [super init]) {
...
- (void)awakeFromNib {
	...
	[outputText setString:[[SentryLogger sharedInstance] string]];
	// setup to receive Event Log messages
	[[NSNotificationCenter defaultCenter] addObserver:self
		selector:@selector(receiveNotification:)
		name:SentryLoggerNotification
		object:[SentryLogger sharedInstance]
		];
    // restore settings
}
...
{    
    ...
    [theWindow saveFrameUsingName:kSentryLogName];
    // remember settings
    // release ourself
    [self autorelease];
}
